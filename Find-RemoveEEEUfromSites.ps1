<#
.SYNOPSIS
    Combined script "Find-EEEUInSites.ps1" to find and "Remove-EEEUFromFileList.ps1" to remove "Everyone Except External Users" (EEEU) group permissions from SharePoint Online and OneDrive sites.
...
.DESCRIPTION
    ...
    When EEEU permissions are found, the script can optionally remove them based on the $removePermissions parameter.
...
.OUTPUTS
    - A CSV file containing all found EEEU occurrences (path: $env:TEMP\Find_Remove_EEEU_In_Sites_[timestamp].csv)
    - A log file documenting the script's execution (path: $env:TEMP\Find_Remove_EEEU_In_Sites_[timestamp].txt)
...
$logFilePath = "$env:TEMP\Find_Remove_EEEU_In_Sites_$startime.txt"
$outputFilePath = "$env:TEMP\Find_Remove_EEEU_In_Sites_$startime.csv"
$debugLogging = $false  # Set to $true for verbose logging, $false for essential logging only
$removePermissions = $true  # Set to $true to remove EEEU permissions when found, $false to only report
...
# Remove EEEU permissions from an object
function Remove-EEEUPermissions {
    param (
    ...
    if (-not $removePermissions) {
        Write-Log "Remove permissions is disabled. Skipping removal for $objectType : $objectName" "DEBUG"
        return $false
    }
    ...
    try {
                ...
                foreach ($role in $rolelevel) {
                    if ($role.Name -ne 'Limited Access') {
                        try {
                            switch ($objectType) {
                                "File" {
                                    Set-PnPListItemPermission -List $object.ParentList.Title -Identity $object.Id -RemoveRole $role.Name -User $roleuser
                                }
                                "Folder" {
                                    Set-PnPListItemPermission -List $object.ParentList.Title -Identity $object.Id -RemoveRole $role.Name -User $roleuser
                                }
                                "List" {
                                    Set-PnPListPermission -Identity $object.Title -RemoveRole $role.Name -User $roleuser
                                }
                                "Web" {
                                    Set-PnPWebPermission -Identity $object -RemoveRole $role.Name -User $roleuser
                                }
                            }
                            
                            Write-Host "Removed Role: $($role.Name) from EEEU in $objectType : $objectName" -ForegroundColor Green
                            Write-Log "Removed Role: $($role.Name) from EEEU in $objectType : $objectName"
                            $removed = $true
                        }
                        catch {
                            Write-Log "Failed to remove role $($role.Name) from $objectType : $objectName - $_" "ERROR"
                        }
                    }
                }
            }
        ...
        Write-Log "Failed to remove EEEU permissions from $objectType : $objectName - $_" "ERROR"
        return $false
    }
}
    ...
    try {
        ...
        if ($Permissions) {
            ...
            if ($eeeuFound) {
                ...
                # Try to remove permissions if enabled
                $removed = Remove-EEEUPermissions -object $web -objectType "Web" -objectName $web.Title
                ...
                if ($removed) {
                    Write-Host "Successfully removed EEEU from Web level on $SiteURL" -ForegroundColor Green
                    Write-Log "Successfully removed EEEU from Web level on $SiteURL"
                }
            }
        }
    }
        ...
        foreach ($list in $lists) {
            ...
            if ($Permissions) {
                ...
                if ($eeeuFound) {
                    ...
                    # Try to remove permissions if enabled
                    $removed = Remove-EEEUPermissions -object $list -objectType "List" -objectName $list.Title
                    ...
                    if ($removed) {
                        Write-Host "Successfully removed EEEU from List: $($list.Title) on $SiteURL" -ForegroundColor Green
                        Write-Log "Successfully removed EEEU from List: $($list.Title) on $SiteURL"
                    }
                }
            }
        }
...
function Find-EEEUinFolders {
    ...
    try {
        ...
        $folderItems = Invoke-WithRetry -ScriptBlock {
            ...
            Get-PnPListItem -List $list -PageSize 500 | Where-Object { $null -ne $_["FileLeafRef"] -and $_["FSObjType"] -eq 1 }
        }
        ...
        foreach ($folderItem in $folderItems) {
            ...
            if ($Permissions) {
                ...
                if ($eeeuFound) {
                    ...
                    # Try to remove permissions if enabled
                    # Ensure ParentList property is loaded
                    Invoke-WithRetry -ScriptBlock {
                        Get-PnPProperty -ClientObject $folderItem -Property ParentList
                    }
                    $removed = Remove-EEEUPermissions -object $folderItem -objectType "Folder" -objectName $folderName
                    ...
                    if ($removed) {
                        Write-Host "Successfully removed EEEU from Folder: $folderName on $SiteURL" -ForegroundColor Green
                        Write-Log "Successfully removed EEEU from Folder: $folderName on $SiteURL"
                    }
                }
            }
        }
            ...
            if ($rootFolder -and $rootFolder.ListItemAllFields) {
                ...
                if ($Permissions) {
                    ...
                    if ($eeeuFound) {
                        ...
                        # Ensure ParentList property is loaded
                        Invoke-WithRetry -ScriptBlock {
                            Get-PnPProperty -ClientObject $rootFolderItem -Property ParentList
                        }
                        $removed = Remove-EEEUPermissions -object $rootFolderItem -objectType "Folder" -objectName $list.Title
                        ...
                        if ($removed) {
                            Write-Host "Successfully removed EEEU from Root Folder: $($list.Title) on $SiteURL" -ForegroundColor Green
                            Write-Log "Successfully removed EEEU from Root Folder: $($list.Title) on $SiteURL"
                        }
                    }
                }
            }
...
# Function to check for EEEU in file-level permissions
    ...
    try {
        ...
        if ($Permissions) {
            ...
            if ($eeeuFound) {
                ...
                $removed = Remove-EEEUPermissions -object $file -objectType "File" -objectName $file.FieldValues.FileLeafRef
                ...
                if ($removed) {
                    Write-Host "Successfully removed EEEU from file: $($file.FieldValues.FileLeafRef) on $SiteURL" -ForegroundColor Green
                    Write-Log "Successfully removed EEEU from file: $($file.FieldValues.FileLeafRef) on $SiteURL"
                }
            }
        }
    }
...
function Write-EEEUOccurrencesToCSV {
    ...
    try {
        ...
        if (-not (Test-Path $filePath) -or -not $Append) {
            # Create empty file with headers - adding Removed column
            "Url,ItemURL,ItemType,RoleNames,OwnerName,OwnerEmail,CreatedDate,Removed" | Out-File -FilePath $filePath
        }
        ...
        foreach ($occurrence in $uniqueOccurrences) {
            ...
            $csvLine = "`"$($occurrence.Url)`",`"$($occurrence.ItemURL)`",`"$($occurrence.ItemType)`",`"$($occurrence.RoleNames)`",`"$($occurrence.OwnerName)`",`"$($occurrence.OwnerEmail)`",`"$($occurrence.CreatedDate)`",`"$($occurrence.Removed)`""
...
function Invoke-SiteAndSubsitesProcessing {
    param (
        [string]$siteURL
    )
    ...
    if (Connect-SharePoint -siteURL $siteURL) {
        ...
        if ($subsites -and $subsites.Count -gt 0) {
            ...
            foreach ($subsite in $subsites) {
                ...
                Invoke-SiteAndSubsitesProcessing -siteURL $subsite.Url
            }
        }
    }
...
Write-Host "Starting Find and Remove EEEU Script" -ForegroundColor Cyan
Write-Host "Remove Permissions Mode: $removePermissions" -ForegroundColor $(if ($removePermissions) { "Red" } else { "Yellow" })
...
if ($removePermissions) {
    Write-Host "Total EEEU permissions removed: $totalRemoved" -ForegroundColor Green
}
else {
    Write-Host "Remove permissions was disabled - no permissions were removed" -ForegroundColor Yellow
}
Write-Host "Results available in: $outputFilePath" -ForegroundColor Cyan
Write-Host "Log file available in: $logFilePath" -ForegroundColor Cyan

Write-Log "EEEU scan and removal completed. Found: $totalFound, Removed: $totalRemoved"
